...
bool libj::parse_subsystem(jcc_job_t &job, std::shared_ptr<libj::Scanner> scanner, std::shared_ptr<libj::StmtNode> &node)
{
    Token tok = scanner->next();
    if (tok.type() != TokenType::Identifier)
    {
        PARMSG(tok, libj::Err::ERROR, feedback[SUBSYSTEM_MISSING_IDENTIFIER]);
        return false;
    }
    ...
    if (tok.type() == TokenType::Punctor && std::get<Punctor>(tok.val()) == Punctor::Colon)
    {
        ...
        if (tok.type() != TokenType::Identifier)
        {
            PARMSG(tok, libj::Err::ERROR, feedback[SUBSYSTEM_EXPECTED_IDENTIFIER]);
            return false;
        }
    ...
    {
        PARMSG(tok, libj::Err::ERROR, feedback[PARSER_EXPECTED_SEMICOLON], tok.serialize().c_str());
        return false;
    }
