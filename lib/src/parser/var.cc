...
bool libj::parse_var(jcc_job_t &job, std::shared_ptr<libj::Scanner> scanner, std::shared_ptr<libj::StmtNode> &node)
{
    Token tok = scanner->next();
    if (tok.type() != TokenType::Identifier)
    {
        PARMSG(tok, libj::Err::ERROR, feedback[VAR_DECL_MISSING_IDENTIFIER]);
        return false;
    }
    ...
    {
        PARMSG(tok, libj::Err::ERROR, feedback[VAR_DECL_MISSING_COLON]);
        return false;
    }
    ...
    {
        PARMSG(tok, libj::Err::ERROR, feedback[VAR_DECL_TYPE_ERR], name.c_str());
        return false;
    }
    ...
    else if (tok.type() == TokenType::Operator && std::get<Operator>(tok.val()) == Operator::Assign)
    {
        ...
        if (tok.type() != TokenType::Punctor || std::get<Punctor>(tok.val()) != Punctor::Semicolon)
        {
            PARMSG(tok, libj::Err::ERROR, feedback[VAR_DECL_MISSING_PUNCTOR], name.c_str());
            return false;
        }
