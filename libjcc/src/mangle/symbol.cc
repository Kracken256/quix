...
static std::string serialize_type(const libjcc::TypeNode *type)
{
    using namespace libjcc;

    static std::map<libjcc::NodeType, std::string> basic_typesmap = {
...
static std::shared_ptr<libjcc::TypeNode> deserialize_type(const std::string &type)
{
    using namespace libjcc;
...
std::string libjcc::Symbol::mangle(std::shared_ptr<libjcc::DeclNode> node, const std::string &prefix)
{
    return mangle(node.get(), prefix);
}

std::string libjcc::Symbol::mangle(const libjcc::DeclNode *node, const std::string &prefix)
{
    std::string res = abiprefix;

    switch (node->ntype)
    {
    case libjcc::NodeType::VarDeclNode:
    {
        res += "v";
        auto var = static_cast<const libjcc::VarDeclNode *>(node);
    ...
    case libjcc::NodeType::LetDeclNode:
    {
        res += "l";
        auto var = static_cast<const libjcc::LetDeclNode *>(node);
        res += wrap_tag(serialize_ns(prefix + var->m_name));
        res += wrap_tag(serialize_type(var->m_type.get()));
    ...
    case libjcc::NodeType::ConstDeclNode:
    {
        res += "c";
        auto var = static_cast<const libjcc::ConstDeclNode *>(node);
        res += wrap_tag(serialize_ns(prefix + var->m_name));
        res += wrap_tag(serialize_type(var->m_type.get()));
...
std::shared_ptr<libjcc::DeclNode> libjcc::Symbol::demangle(const std::string &mangled)
{
    std::string input;
    ...
    try
    {
        ...
        case 'v':
        {
            ...
            auto var = std::make_shared<libjcc::VarDeclNode>();
        ...
        case 'l':
        {
            ...
            auto let = std::make_shared<libjcc::LetDeclNode>();
        ...
        case 'c':
        {
            ...
            auto con = std::make_shared<libjcc::ConstDeclNode>();
