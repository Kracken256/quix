...
bool libjcc::parse_const(jcc_job_t &job, std::shared_ptr<libjcc::Scanner> scanner, std::shared_ptr<libjcc::StmtNode> &node)
{
    Token tok = scanner->next();
    if (tok.type() != TokenType::Identifier)
    {
        PARMSG(tok, libjcc::Err::ERROR, feedback[CONST_DECL_MISSING_IDENTIFIER]);
        return false;
    }
    ...
    {
        PARMSG(tok, libjcc::Err::ERROR, feedback[CONST_DECL_MISSING_COLON]);
        return false;
    }
    ...
    {
        PARMSG(tok, libjcc::Err::ERROR, feedback[CONST_DECL_TYPE_ERR], name.c_str());
        return false;
    }
    ...
    else if (tok.type() == TokenType::Operator && std::get<Operator>(tok.val()) == Operator::Assign)
    {
        ...
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[CONST_DECL_INIT_ERR], name.c_str());
            return false;
        }
        ...
        if (tok.type() != TokenType::Punctor || std::get<Punctor>(tok.val()) != Punctor::Semicolon)
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[CONST_DECL_MISSING_PUNCTOR], name.c_str());
            return false;
        }
