...
static bool fn_get_property(jcc_job_t &job, std::shared_ptr<libjcc::Scanner> scanner, std::shared_ptr<libjcc::FunctionDeclNode> &node, GetPropState &state)
{
    ...
    if (tok.type() == TokenType::Keyword && std::get<Keyword>(tok.val()) == Keyword::Nothrow)
    {
        ...
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[FN_NO_THROW_ALREADY_SPECIFIED], tok.serialize().c_str());
            return false;
        }
    ...
    if (tok.type() == TokenType::Keyword && std::get<Keyword>(tok.val()) == Keyword::Foreign)
    {
        ...
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[FN_FOREIGN_ALREADY_SPECIFIED], tok.serialize().c_str());
            return false;
        }
        node->m_foreign = true;
        scanner->next();
    ...
    if (tok.type() == TokenType::Keyword && std::get<Keyword>(tok.val()) == Keyword::Tsafe)
    {
        if (state.did_tsafe)
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[FN_THREAD_SAFE_ALREADY_SPECIFIED], tok.serialize().c_str());
            return false;
        }
    ...
    if (tok.type() == TokenType::Keyword && std::get<Keyword>(tok.val()) == Keyword::Pure)
    {
        if (state.did_pure)
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[FN_PURE_ALREADY_SPECIFIED], tok.serialize().c_str());
            return false;
        }

        if (state.did_impure)
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[FN_PURE_AND_IMPURE_SPECIFIED], tok.serialize().c_str());
            return false;
        }
        node->m_pure = true;
        scanner->next();
    ...
    if (tok.type() == TokenType::Keyword && std::get<Keyword>(tok.val()) == Keyword::Impure)
    {
        if (state.did_impure)
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[FN_IMPURE_ALREADY_SPECIFIED], tok.serialize().c_str());
            return false;
        }
        if (state.did_pure)
        {
...
static bool parse_fn_parameter(jcc_job_t &job, std::shared_ptr<libjcc::Scanner> scanner, FunctionDeclNode::Param &param)
{
    return false;
}

bool libjcc::parse_function(jcc_job_t &job, std::shared_ptr<libjcc::Scanner> scanner, std::shared_ptr<libjcc::StmtNode> &node)
{
    ...
    {
        PARMSG(tok, libjcc::Err::ERROR, feedback[FN_EXPECTED_IDENTIFIER], tok.serialize().c_str());
        return false;
    }
    ...
    {
        PARMSG(tok, libjcc::Err::ERROR, feedback[FN_EXPECTED_OPEN_PAREN], tok.serialize().c_str());
        return false;
    }
    ...
    while ((tok = scanner->next()).type() != TokenType::Punctor || std::get<Punctor>(tok.val()) != Punctor::CloseParen)
    {
        ...
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[FN_EXPECTED_CLOSE_PAREN_OR_COMMA], tok.serialize().c_str());
            return false;
        }
    }
    ...
    {
        PARMSG(tok, libjcc::Err::ERROR, feedback[FN_EXPECTED_OPEN_BRACE], tok.serialize().c_str());
        return false;
    }
