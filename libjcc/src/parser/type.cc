...
bool libjcc::parse_type(jcc_job_t &job, std::shared_ptr<libjcc::Scanner> scanner, std::shared_ptr<libjcc::TypeNode> &node)
{
    ...
    else if (tok.type() == TokenType::Punctor && std::get<Punctor>(tok.val()) == Punctor::OpenBracket)
    {
        ...
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[TYPE_EXPECTED_TYPE]);
            return false;
        }
        ...
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[TYPE_EXPECTED_SEMICOLON]);
            return false;
        }
        ...
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[TYPE_EXPECTED_CONST_EXPR]);
            return false;
        }
        ...
        {
            PARMSG(tok, libjcc::Err::ERROR, feedback[TYPE_EXPECTED_CLOSE_BRACKET]);
            return false;
        }
    ...
    {
        PARMSG(tok, libjcc::Err::ERROR, feedback[TYPE_EXPECTED_IDENTIFIER_OR_BRACKET]);
        return false;
    }
}
