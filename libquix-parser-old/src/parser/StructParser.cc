...
static bool parse_struct_field(quixcc_cc_job_t &job, Scanner *scanner, CompositeField **node) {
  /**
...
bool parser::parse_struct(quixcc_cc_job_t &job, Scanner *scanner, Stmt **node) {
  /**
  ...
  { /* The compiler may automatically generate traits for the definition */
    tok = scanner->peek();
    if (!job.has("-fno-auto-impl", "struct")) {
