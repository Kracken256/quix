...
static bool parse_region_field(quixcc_cc_job_t &job, Scanner *scanner, CompositeField **node) {
  /**
  ...
  { /* Optional default value */
    if (!tok.is<Operator>(OpAssign)) {
      LOG(ERROR) << core::feedback[REGION_FIELD_MISSING_PUNCTOR] << name << tok << std::endl;
      return false;
    }
...
bool parser::parse_region(quixcc_cc_job_t &job, Scanner *scanner, Stmt **node) {
  /**
    ...
    if (tok.is<Keyword>(Keyword::Impl)) {
      ...
      { /* The implementation list should be enclosed in square brackets ex: [abc, hello] */
        if (!tok.is<Punctor>(OpenBracket)) {
          LOG(ERROR) << core::feedback[REGION_DEF_EXPECTED_OPEN_BRACKET] << tok << std::endl;
          return false;
        }
      }
      ...
      while (true) {
        ...
        if (!tok.is(tName)) {
          LOG(ERROR) << core::feedback[REGION_DEF_EXPECTED_IDENTIFIER] << tok << std::endl;
          return false;
        }
