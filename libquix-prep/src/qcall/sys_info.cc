...
/// TODO: Implement the info log
static thread_local std::ostream& info_log = std::clog;
...
int qcall::sys_info(lua_State* L) {
  ...
  for (int i = 1; i <= nargs; i++) {
    if (lua_isstring(L, i)) {
      info_log << lua_tostring(L, i);
    } else if (lua_isnumber(L, i)) {
      info_log << lua_tonumber(L, i);
    } else if (lua_isboolean(L, i)) {
      info_log << (lua_toboolean(L, i) ? "true" : "false");
    } else {
