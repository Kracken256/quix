...
const char *Node::type_name(qxir_ty_t type) {
#define NAMEOF_ROW(__name) \
  ...
  qassert(names.size() == QIR_NODE_COUNT, "Polymorphic type size lookup table is incomplete");
...
#define SIZEOF_ROW(__type) \
  ...
  qassert(sizes.size() == QIR_NODE_COUNT, "Polymorphic type size lookup table is incomplete");
...
qxir_ty_t Node::this_typeid() const {
#define TYPEID_ROW(__type, __name) \
  ...
  qassert(typeid_map.size() == QIR_NODE_COUNT);
...
bool Node::is(qxir_ty_t type) const { return type == this_typeid(); }
...
#define INDENT_STEP 1
  ...
  if (binary_repr) {
    qxir_brepr(this, minify, &qxir_arena.get(), &outbuf, &len);
  } else {
    outbuf = (uint8_t *)qxir_repr(this, minify, INDENT_STEP, &qxir_arena.get(), &len);
  }
...
bool Type::is_array() const {
  ...
  return this_typeid() == QIR_NODE_ARRAY_TY;
}
...
bool Type::is_vector() const {
  ...
  return this_typeid() == QIR_NODE_VECTOR_TY;
}
...
bool Type::is_tuple() const {
  ...
  return this_typeid() == QIR_NODE_TUPLE_TY;
}
...
bool Type::is_set() const {
  ...
  return this_typeid() == QIR_NODE_SET_TY;
}
...
bool Type::is_map() const {
  ...
  return this_typeid() == QIR_NODE_MAP_TY;
}
...
bool Type::is_pointer() const {
  ...
  return this_typeid() == QIR_NODE_PTR_TY;
}
...
bool Type::is_function() const {
  ...
  return this_typeid() == QIR_NODE_FN_TY;
}
...
bool Type::is_union() const {
  ...
  return this_typeid() == QIR_NODE_UNION_TY;
}
...
bool Type::is_void() const {
  ...
  return this_typeid() == QIR_NODE_VOID_TY;
}
...
bool Type::is_bool() const {
  ...
  return this_typeid() == QIR_NODE_U1_TY;
}

bool Type::is_mutable() const { return this_typeid() == QIR_NODE_MUT_TY; }
...
bool Type::is_string() const {
  ...
  return this_typeid() == QIR_NODE_STRING_TY;
}
...
void SubsystemDecl::add_dep(qxir::String dep) { m_deps.insert(dep); }
void qxir::SubsystemDecl::add_deps(const qxir::SubsystemDeps &deps) {
...
LIB_EXPORT qxir_node_t *qxir_alloc(qxir_ty_t type, qcore_arena_t *arena) {
  if (!arena) {
    arena = &qxir_arena.get();
  }
  ...
  switch (type) {
    case QIR_NODE_STMT:
      qcore_panic("qxir_alloc(): QIR_NODE_STMT is not valid here");
    case QIR_NODE_TYPE:
      qcore_panic("qxir_alloc(): QIR_NODE_TYPE is not valid here");
    case QIR_NODE_DECL:
      qcore_panic("qxir_alloc(): QIR_NODE_DECL is not valid here");
    case QIR_NODE_EXPR:
      qcore_panic("qxir_alloc(): QIR_NODE_EXPR is not valid here");
    case QIR_NODE_CEXPR:
      node = ConstExpr::get();
      break;
    case QIR_NODE_UNRES_TY:
