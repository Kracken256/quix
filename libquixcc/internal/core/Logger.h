...
typedef std::function<std::string(const std::string &, E type, const Token &)>
    LogFormatFunc;
 ...
 public:
  ...
  Logger &operator+=(const T &msg) {
    ...
    if (!m_replacement_idx.empty()) {
      ...
      if (m_replacement_idx.empty()) {
        for (auto &part : m_parts) m_buf << part;
        m_parts = {};
      }
      return *this;
    }
    ...
    if (start < s.length()) m_parts.push_back(s.substr(start));
...
class LoggerGroup {
  ...
  static std::string format_message_ansi(const std::string &message, E type,
                                         const Token &tok);
  static std::string format_message_nocolor(const std::string &message, E type,
                                            const Token &tok);

  static inline std::string format_message(const std::string &message, E type,
                                           const Token &tok) {
 ...
 public:
  ...
  inline void setup(quixcc_job_t &job) {
    ...
    for (auto &logger : m_loggers) {
      logger.second->conf(
          [&job](const std::string &message, E type) {
