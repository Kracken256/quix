...
namespace libquixcc {
  ...
  public:
      ...
      template <typename T> bool is(const T *other) const {
#if !defined(NDEBUG)
        uintptr_t p = reinterpret_cast<uintptr_t>(this);
        if (p == 0) {
          LOG(FATAL) << "Invalid cast from nullptr to `" << typeid(T).name() << "`" << std::endl;
        ...
        if (!other) {
          LOG(FATAL) << "Invalid cast from `" << typeid(*this).name() << "` to nullptr"
                     << std::endl;
          throw core::Exception();
        }
#endif
      ...
      bool print(std::ostream &os, PState &state) const { return print_impl(os, state); }
  ...
  public:
      ...
      template <typename T> Hasher &add(const T value) {
        std::stringstream ss;
        PState state;
      ...
      template <typename T> Hasher &add(const std::vector<T> &values) {
      ...
      template <typename T> Hasher &add(const std::set<T> &values) {
        for (const auto &value : values) {
          add(value);
        }
        return *this;
    ...
    template <auto T, typename U> class IRModule {
  protected:
      std::string m_name;
      U m_root = nullptr;
