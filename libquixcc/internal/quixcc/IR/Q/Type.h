...
public:
    ...
    size_t bitcount() const override { return libquixcc::quixcc::g_target_word_size * 8; }
...
protected:
    ...
    Array(Type *type, uint64_t size) : type(type), size(size) { ntype = (int)QType::Array; }
...
protected:
    ...
    FType(std::vector<Type *> params, Type *ret, bool variadic, bool pure, bool thread_safe,
          bool foreign, bool nothrow)
        : params(params), ret(ret), m_variadic(variadic), m_pure(pure), m_thread_safe(thread_safe),
          m_foreign(foreign), m_noexcept(nothrow) {
      ntype = (int)QType::FType;
    }

public:
    static FType *create(std::vector<Type *> params, Type *ret, bool variadic = false,
                         bool pure = false, bool thread_safe = false, bool foreign = false,
                         bool nothrow = false);
...
public:
    static Region *create(std::string name, std::vector<Type *> fields, bool packed, bool ordered);
    size_t bitcount() const override {
...
protected:
    ...
    Union(std::string name, std::vector<Type *> fields) : name(name), fields(fields) {
...
protected:
    ...
    IntrinsicType(QIntrinsicType name) : name(name) { ntype = (int)QType::IntrinsicType; }
