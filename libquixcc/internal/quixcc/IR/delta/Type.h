...
protected:
    ...
    PacketDef(std::vector<std::pair<std::string, const Type *>> fields, std::string name)
        : fields(fields), name(name) {
      ntype = (int)NodeType::PacketDef;
    }

public:
    static const PacketDef *create(std::vector<std::pair<std::string, const Type *>> fields,
                                   std::string name);
