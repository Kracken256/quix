...
public:
    inline Logger(E level)
        : m_dispatch(nullptr), m_fmt(nullptr), m_level(level), m_mode(log::format) {}
  ...
  class LoggerGroup {
    ...
    static void push_message_to_job(quixcc_cc_job_t &job, E type, const std::string &message);
    ...
    static inline std::string format_message(const std::string &message, E type, const Token &tok) {
      return is_color_enabled() ? format_message_ansi(message, type, tok)
                                : format_message_nocolor(message, type, tok);
    }
...
public:
    ...
    inline void setup(quixcc_cc_job_t &job) {
      ...
      for (auto &logger : m_loggers) {
        logger.second->conf(
            [&job](const std::string &message, E type) { push_message_to_job(job, type, message); },
            format_message);
      }

      m_hole->conf(
          [](const std::string &message, E type) {
            (void)message;
            (void)type;
          },
  ...
  template <class T> Logger &operator<<(Logger &logger, const T &msg) {
  ...
  static inline void operator<<(Logger &log, std::ostream &(*var)(std::ostream &)) {
    (void)var;
    log.flush();
  }
...
#define LOGGER_SETUP() thread_local libquixcc::LoggerGroup libquixcc::_G_loggerGrp;
  ...
  static inline void LoggerConfigure(quixcc_cc_job_t &job) { _G_loggerGrp.setup(job); }
