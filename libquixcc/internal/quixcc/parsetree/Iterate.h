...
namespace libquixcc {
  namespace traversal {
    ...
    typedef std::pair<TraversePtrType, std::variant<std::shared_ptr<libquixcc::ParseNode> *,
                                                    libquixcc::ParseNode **>>
        TraversePtr;

    typedef std::function<void(const std::vector<std::string> &_namespace,
                               const std::vector<std::string> &_scope, libquixcc::ParseNode *,
                               TraversePtr)>
  ...
  public:
      ParseTreePreorder(ParseTreePreorderCallback callback) : m_callback(callback) {}

      template <typename T> inline size_t next(T node) {
