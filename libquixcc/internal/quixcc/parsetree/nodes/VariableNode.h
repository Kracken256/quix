...
public:
    VarDeclNode()
        : m_name(""), m_type(nullptr), m_init(nullptr), m_is_mut(false), m_is_thread_local(false),
          m_is_static(false), m_is_deprecated(false) {
      ntype = NodeType::VarDeclNode;
    }
    VarDeclNode(const std::string &name, std::shared_ptr<TypeNode> type,
                const std::shared_ptr<ExprNode> init, bool is_mut, bool is_thread_local,
                bool is_static, bool is_deprecated = false)
        : m_name(name), m_type(type), m_init(init), m_is_mut(is_mut),
          m_is_thread_local(is_thread_local), m_is_static(is_static),
...
public:
    ...
    LetDeclNode(const std::string &name, std::shared_ptr<TypeNode> type,
                ...
                bool is_thread_local = false, bool is_static = false, bool is_deprecated = false)
        : m_name(name), m_type(type), m_init(init), m_is_mut(is_mut),
          m_is_thread_local(is_thread_local), m_is_static(is_static),
          m_is_deprecated(is_deprecated) {
      ntype = NodeType::LetDeclNode;
...
public:
    ConstDeclNode() : m_name(""), m_type(nullptr), m_init(nullptr), m_is_deprecated(false) {
      ntype = NodeType::ConstDeclNode;
    }
    ConstDeclNode(const std::string &name, std::shared_ptr<TypeNode> type,
                  const std::shared_ptr<ExprNode> init, bool is_deprecated = false)
        : m_name(name), m_type(type), m_init(init), m_is_deprecated(is_deprecated) {
      ntype = NodeType::ConstDeclNode;
    }
