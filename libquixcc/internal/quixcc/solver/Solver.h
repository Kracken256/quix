...
namespace libquixcc {
  namespace solver {
    typedef bool (*SolPassFunc)(quixcc_cc_job_t &job, std::unique_ptr<ir::q::QModule> &ir);
  ...
  public:
      SolPass(std::string name, std::string description, SolPassFunc func,
          ...
          : m_name(name), m_description(description), m_author(author), m_version(version),
            m_prepass_dependencies(prepass_dependencies),
            m_postpass_dependencies(postpass_dependencies), m_prepass_suggests(prepass_suggests),
            m_postpass_suggests(postpass_suggests), m_func(func) {
        derive_uuid();
      }
      ...
      inline const std::vector<SolPassUUID> &prepass_suggests() const { return m_prepass_suggests; }
  ...
  public:
      SolPassManager(std::shared_ptr<SolPassRegistry> registry) : m_registry(registry) {}
