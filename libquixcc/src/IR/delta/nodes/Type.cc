...
bool libquixcc::ir::delta::Void::verify_impl() const { return true; }

boost::uuids::uuid libquixcc::ir::delta::Ptr::hash_impl() const {
  return Hasher().gettag().add(type).hash();
}

bool libquixcc::ir::delta::Ptr::verify_impl() const { return type->verify(); }
...
bool libquixcc::ir::delta::Packet::verify_impl() const { return def->verify(); }
