...
LIB_CXX_EXPORT quixcc::TargetTriple::TargetTriple(const char *_triple) {
  if (!_triple)
    throw TargetTripleException(
        "Invalid or unsupported LLVM target triple: (null)");
  ...
  if (triple[0] == '\0') {
    ...
    if (!llvm::TargetRegistry::lookupTarget(triple, err))
      throw TargetTripleException(
          "Invalid or unsupported LLVM target triple: " + triple);
...
LIB_CXX_EXPORT quixcc::Compiler &quixcc::Compiler::run(size_t max_threads) {
  ...
  for (auto job : this->m_jobs) {
    ...
    for (i = 0; i < status->m_count; i++) {
      ...
      this->m_messages.push_back(
          std::make_pair(feedback->message, feedback->m_level));
    }
...
LIB_CXX_EXPORT quixcc::Compiler &quixcc::Compiler::puts(std::ostream &normal,
                                                        std::ostream &error) {
  size_t i;
...
LIB_CXX_EXPORT quixcc::CompilerBuilder &quixcc::CompilerBuilder::add_code(
...
LIB_CXX_EXPORT quixcc::CompilerBuilder &quixcc::CompilerBuilder::define(
    const std::string &name, const std::string &value) {
  m_flags.push_back("-D" + name + "=" + value);
  return *this;
}

LIB_CXX_EXPORT quixcc::CompilerBuilder &quixcc::CompilerBuilder::undefine(
    const std::string &name) {
  m_flags.push_back("-U" + name);
  return *this;
}

LIB_CXX_EXPORT quixcc::CompilerBuilder &quixcc::CompilerBuilder::set_flag(
...
LIB_CXX_EXPORT quixcc::CompilerBuilder &quixcc::CompilerBuilder::set_debug(
    bool debug) {
...
LIB_CXX_EXPORT quixcc::CompilerBuilder &quixcc::CompilerBuilder::opt(
    const std::string &flag) {
  m_flags.push_back(flag);
  return *this;
}

LIB_CXX_EXPORT quixcc::CompilerBuilder &quixcc::CompilerBuilder::disregard(
    bool disregard) {
...
LIB_CXX_EXPORT quixcc::CompilerBuilder &quixcc::CompilerBuilder::cpu(
    quixcc::CPU cpu) {
  m_cpu = cpu;
  return *this;
}
