...
LIB_CXX_EXPORT quixcc::CompilerBuilder &quixcc::CompilerBuilder::define(const std::string &name,
...
LIB_CXX_EXPORT quixcc::Compiler quixcc::CompilerBuilder::build() {
  ...
  for (auto &entry : m_files) {
    ...
    if (!quixcc_cc_target(job, m_target.triple().c_str()))
      throw TargetTripleException("Invalid or unsupported LLVM target triple: " +
                                  m_target.triple());
