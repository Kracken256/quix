...
void libquixcc::mutate::DiscoverNamedConstructs(quixcc_job_t *job, std::shared_ptr<libquixcc::BlockNode> ast)
{
    ...
    ast->dfs_preorder(traversal::ParseTreeTraversalState(
        [&named_construct_map, &named_types_map](const std::vector<std::string> &_namespace, const std::vector<std::string> &_scope, libquixcc::ParseNode *parent, traversal::TraversePtr node)
        {
