...
static void resolve_user_type_nodes(quixcc_job_t *job,
  ...
  ast->dfs_preorder(traversal::ParseTreeTraversalState(
      [job](const std::vector<std::string> &_namespace,
        ...
        switch (named_type->ntype) {
          case NodeType::StructDefNode:
            type =
                std::static_pointer_cast<libquixcc::StructDefNode>(named_type)
                    ->get_type();
            break;
          case NodeType::RegionDefNode:
            type =
                std::static_pointer_cast<libquixcc::RegionDefNode>(named_type)
                    ->get_type();
            break;
      ...
      [job](const std::vector<std::string> &_namespace,
        ...
        auto enum_field = std::static_pointer_cast<libquixcc::EnumFieldNode>(
            job->m_inner.m_named_construsts[std::make_pair(
                NodeType::EnumFieldNode, ident->m_name)]);
      ...
      [job](const std::vector<std::string> &_namespace,
        ...
        switch (call->m_callee->ntype) {
          case NodeType::IdentifierNode:
            callee_name = std::static_pointer_cast<libquixcc::IdentifierNode>(
                              call->m_callee)
                              ->m_name;
            break;
        ...
        while (!tmp.empty()) {
          ...
          if (job->m_inner.m_named_construsts.contains(
            ...
            auto func_decl =
                std::static_pointer_cast<libquixcc::FunctionDeclNode>(
                    job->m_inner.m_named_construsts[std::make_pair(
        ...
        if (!job->m_inner.m_named_construsts.contains(
          ...
          LOG(ERROR) << feedback[UNRESOLVED_FUNCTION] << callee_name
                     << std::endl;
          return;
        }
