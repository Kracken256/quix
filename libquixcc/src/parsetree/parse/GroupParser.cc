...
static bool parse_group_field(quixcc_job_t &job, libquixcc::Scanner *scanner, std::shared_ptr<GroupFieldNode> &node)
{
...
bool libquixcc::parse_group(quixcc_job_t &job, libquixcc::Scanner *scanner, std::shared_ptr<libquixcc::StmtNode> &node)
{
    Token tok = scanner->next();
    if (tok.type != TT::Identifier)
    {
