...
bool libquixcc::parse_let(quixcc_job_t &job, libquixcc::Scanner *scanner, std::vector<std::shared_ptr<libquixcc::StmtNode>> &nodes)
{
    ...
    if (tok.is<Punctor>(Punctor::OpenBracket))
    {
        ...
        */
        ...
        while (true)
        {
            ...
            {
                LOG(ERROR) << feedback[LET_DECL_MISSING_PUNCTOR] << decl.first << tok << std::endl;
                return false;
            }
        }
    }
    ...
    else if (tok.is<Operator>(Operator::Assign))
    {
        ...
        if (!tok.is<Punctor>(Punctor::Semicolon))
        {
            LOG(ERROR) << feedback[LET_DECL_MISSING_PUNCTOR] << decls[0].first << tok << std::endl;
            return false;
        }
