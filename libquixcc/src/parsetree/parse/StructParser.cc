...
static bool parse_struct_field(quixcc_job_t &job, libquixcc::Scanner *scanner, std::shared_ptr<StructFieldNode> &node)
{
...
bool libquixcc::parse_struct(quixcc_job_t &job, libquixcc::Scanner *scanner, std::shared_ptr<libquixcc::StmtNode> &node)
{
    Token tok = scanner->next();
    if (tok.type != TT::Identifier)
    {
    ...
    while (true)
    {
        ...
        else if (tok.is<Keyword>(Keyword::Static))
        {
            ...
            if (!tok.is<Keyword>(Keyword::Fn))
            {
                LOG(ERROR) << feedback[STRUCT_DEF_EXPECTED_FN] << tok << std::endl;
                return false;
            }
