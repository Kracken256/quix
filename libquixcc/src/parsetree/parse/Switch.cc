...
bool libquixcc::parse_switch(quixcc_job_t &job, libquixcc::Scanner *scanner,
  ...
  if (!tok.is<Punctor>(Punctor::OpenBrace)) {
    LOG(ERROR) << feedback[SWITCH_EXPECTED_LEFT_BRACE] << tok.serialize() << tok
               << std::endl;
    return false;
  }
  ...
  while (true) {
    ...
    if (tok.is<Keyword>(Keyword::Default)) {
      scanner->next();
      if (default_case) {
        LOG(ERROR) << feedback[SWITCH_MULTIPLE_DEFAULT] << tok.serialize()
                   << tok << std::endl;
        return false;
      }
      ...
      if (!tok.is<Punctor>(Punctor::Colon)) {
        LOG(ERROR) << feedback[SWITCH_EXPECTED_COLON] << tok.serialize() << tok
                   << std::endl;
        return false;
      }
    ...
    if (!tok.is<Keyword>(Keyword::Case)) {
      LOG(ERROR) << feedback[SWITCH_EXPECTED_CASE] << tok.serialize() << tok
                 << std::endl;
      return false;
    }
    scanner->next();
  ...
  tok = scanner->next();
  if (!tok.is<Punctor>(Punctor::CloseBrace)) {
    LOG(ERROR) << feedback[SWITCH_EXPECTED_RIGHT_BRACE] << tok.serialize()
               << tok << std::endl;
    return false;
  }
