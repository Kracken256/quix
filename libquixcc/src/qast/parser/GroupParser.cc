...
static bool parse_group_field(quixcc_cc_job_t &job, Scanner *scanner, CompositeField **node) {
  /**
  ...
  { /*First token is the field name */
    tok = scanner->next();
    if (!tok.is(tName)) {
      LOG(ERROR) << core::feedback[GROUP_FIELD_MISSING_IDENTIFIER] << tok << std::endl;
      return false;
    }
    name = tok.as_string();
  }
  ...
  { /* Next token should be a colon */
    ...
    if (!tok.is<Punctor>(Colon)) {
      LOG(ERROR) << core::feedback[GROUP_FIELD_MISSING_COLON] << tok << std::endl;
      return false;
    }
  }
  ...
  { /* Next section should be the field type */
    if (!parse_type(job, scanner, &type)) {
      LOG(ERROR) << core::feedback[GROUP_FIELD_TYPE_ERR] << name << tok << std::endl;
      return false;
    }
  }
  ...
  { /* Optional default value */
    if (!tok.is<Operator>(OpAssign)) {
      LOG(ERROR) << core::feedback[GROUP_FIELD_MISSING_PUNCTOR] << name << tok << std::endl;
      return false;
    }
...
bool parser::parse_group(quixcc_cc_job_t &job, Scanner *scanner, Stmt **node) {
  /**
  ...
  { /* First token should be the name of the definition */
    tok = scanner->next();
    if (!tok.is(tName)) {
      LOG(ERROR) << core::feedback[GROUP_DECL_MISSING_IDENTIFIER] << tok << std::endl;
      return false;
    }
  ...
  { /* Next token should be an open curly bracket */
    ...
    if (!tok.is<Punctor>(OpenBrace)) {
      LOG(ERROR) << core::feedback[GROUP_DEF_EXPECTED_OPEN_BRACE] << tok << std::endl;
      return false;
    }
  }
  ...
  while (true) {
    ...
    if (tok.is<Keyword>(Keyword::Fn)) {
      ...
      if (!tok.is<Keyword>(Keyword::Fn)) {
        LOG(ERROR) << core::feedback[GROUP_DEF_EXPECTED_FN] << tok << std::endl;
        return false;
      }
  ...
  { /* The compiler may automatically generate traits for the definition */
    tok = scanner->peek();
    if (!job.has("-fno-auto-impl", "group")) {
    ...
    if (tok.is<Keyword>(Keyword::Impl)) {
      ...
      { /* The implementation list should be enclosed in square brackets ex: [abc, hello] */
        if (!tok.is<Punctor>(OpenBracket)) {
          LOG(ERROR) << core::feedback[GROUP_DEF_EXPECTED_OPEN_BRACKET] << tok << std::endl;
          return false;
        }
      }
      ...
      while (true) {
        ...
        if (tok.type() != tName) {
          LOG(ERROR) << core::feedback[GROUP_DEF_EXPECTED_IDENTIFIER] << tok << std::endl;
          return false;
        }
