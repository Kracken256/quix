...
bool libquixcc::qast::parser::parse_let(quixcc_cc_job_t &job, libquixcc::Scanner *scanner,
                                        StmtListItems &nodes) {
  ...
  if (tok.is<Punctor>(OpenBracket)) {
    ...
    while (true) {
      ...
      if (tok.is<Punctor>(Comma))
        ...
        LOG(ERROR) << core::feedback[LET_DECL_MISSING_PUNCTOR] << decl.first << tok << std::endl;
        return false;
      }
    }
  ...
  if (tok.is<Punctor>(Semicolon)) {
    for (auto &decl : decls)
      nodes.push_back(LetDecl::get(decl.first, decl.second, nullptr));
    ...
    tok = scanner->next();
    if (!tok.is<Punctor>(Semicolon)) {
      LOG(ERROR) << core::feedback[LET_DECL_MISSING_PUNCTOR] << decls[0].first << tok << std::endl;
      return false;
    }
