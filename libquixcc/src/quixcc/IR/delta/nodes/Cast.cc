...
boost::uuids::uuid libquixcc::ir::delta::SCast::hash_impl() const {
...
bool libquixcc::ir::delta::IPtrCast::verify_impl() const {
  return type->verify() && value->verify();
}

boost::uuids::uuid libquixcc::ir::delta::Bitcast::hash_impl() const {
  return Hasher().gettag().add(type).add(value).hash();
}

bool libquixcc::ir::delta::Bitcast::verify_impl() const {
  return type->verify() && value->verify();
}