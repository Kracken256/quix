...
static auto conv(const ir::q::RootNode *n, DState &state) -> DResult {
  ...
  for (auto it = state.oop.begin(); it != state.oop.end();) {
    ...
    for (auto method : methods)
      bins[Type::FnDecl].push_back(method);
  ...
  for (auto typedefs : bins[Type::TypeDecl])
    sorted.push_back(typedefs);
  for (auto globals : bins[Type::GlobalDecl])
    sorted.push_back(globals);
  for (auto functions : bins[Type::FnForward])
    sorted.push_back(functions);
  for (auto functions : bins[Type::FnDecl])
    sorted.push_back(functions);

  return RootNode::create(sorted);
}

static auto conv(const ir::q::I1 *n, DState &state) -> DResult { return I1::create(); }

static auto conv(const ir::q::I8 *n, DState &state) -> DResult { return I8::create(); }

static auto conv(const ir::q::I16 *n, DState &state) -> DResult { return I16::create(); }

static auto conv(const ir::q::I32 *n, DState &state) -> DResult { return I32::create(); }

static auto conv(const ir::q::I64 *n, DState &state) -> DResult { return I64::create(); }

static auto conv(const ir::q::I128 *n, DState &state) -> DResult { return I128::create(); }

static auto conv(const ir::q::U8 *n, DState &state) -> DResult { return U8::create(); }

static auto conv(const ir::q::U16 *n, DState &state) -> DResult { return U16::create(); }

static auto conv(const ir::q::U32 *n, DState &state) -> DResult { return U32::create(); }

static auto conv(const ir::q::U64 *n, DState &state) -> DResult { return U64::create(); }

static auto conv(const ir::q::U128 *n, DState &state) -> DResult { return U128::create(); }

static auto conv(const ir::q::F32 *n, DState &state) -> DResult { return F32::create(); }

static auto conv(const ir::q::F64 *n, DState &state) -> DResult { return F64::create(); }

static auto conv(const ir::q::Void *n, DState &state) -> DResult { return Void::create(); }
...
static auto conv(const ir::q::Opaque *n, DState &state) -> DResult { return Void::create(); }
...
static auto conv(const ir::q::For *n, DState &state) -> DResult {
  ...
  if (n->init)
    init = conv(n->init, state)[0]->as<Value>();
  if (n->cond)
    cond = conv(n->cond, state)[0]->as<Expr>();
  if (n->step)
    step = conv(n->step, state)[0]->as<Expr>();
  if (n->body)
    body = conv(n->body, state)[0];

  if (!init)
    init = Number::create("1");
  if (!cond)
    cond = Number::create("1");
  if (!step)
    step = Number::create("1");
...
static auto conv(const ir::q::Break *n, DState &state) -> DResult { return Break::create(); }

static auto conv(const ir::q::Continue *n, DState &state) -> DResult { return Continue::create(); }
...
bool libquixcc::ir::delta::IRDelta::from_qir(quixcc_cc_job_t *job,
