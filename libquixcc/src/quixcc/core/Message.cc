...
void libquixcc::LoggerGroup::push_message_to_job(quixcc_cc_job_t &job, libquixcc::E type,
  ...
  job.m_result.m_messages = (quixcc_msg_t **)realloc(
      job.m_result.m_messages, (job.m_result.m_count + 1) * sizeof(quixcc_msg_t *));
...
bool libquixcc::LoggerGroup::is_color_enabled() {
  ...
  static bool _G_is_color_enabled =
      (getenv("QUIXCC_COLOR") == nullptr) || (std::string(getenv("QUIXCC_COLOR")) == "1");
  return _G_is_color_enabled;
#endif
}

std::string libquixcc::LoggerGroup::format_message_ansi(const std::string &message,
                                                        libquixcc::E type,
...
std::string libquixcc::LoggerGroup::format_message_nocolor(const std::string &message,
                                                           libquixcc::E type,
                                                           const libquixcc::Token &tok) {
  std::stringstream msg;
