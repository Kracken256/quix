...
static bool parse_group_field(quixcc_cc_job_t &job, libquixcc::Scanner *scanner,
  ...
  if (!parse_type(job, scanner, type)) {
    LOG(ERROR) << core::feedback[GROUP_FIELD_TYPE_ERR] << name << tok
               << std::endl;
    return false;
  }
  ...
  if (tok.is<Punctor>(Comma)) {
    ...
    LOG(ERROR) << core::feedback[GROUP_FIELD_MISSING_PUNCTOR] << name << tok
               << std::endl;
    return false;
  }
...
bool libquixcc::parse_group(quixcc_cc_job_t &job, libquixcc::Scanner *scanner,
  ...
  if (!tok.is<Punctor>(OpenBrace)) {
    LOG(ERROR) << core::feedback[GROUP_DEF_EXPECTED_OPEN_BRACE] << tok
               << std::endl;
    return false;
  }
  ...
  while (true) {
    ...
    if (tok.is<Keyword>(Keyword::Fn)) {
      ...
      if (!parse_group_field(job, scanner, field)) return false;
  ...
  if (tok.is<Keyword>(Keyword::Impl)) {
    ...
    if (!tok.is<Punctor>(OpenBracket)) {
      LOG(ERROR) << core::feedback[GROUP_DEF_EXPECTED_OPEN_BRACKET] << tok
                 << std::endl;
      return false;
    }
