...
PrepEngine::Entry PrepEngine::build_statics_decl() {
  ...
  if (job->version) {
    declcode << "@use \"v" << job->version->first << "." << job->version->second << "\";\n";
  } else {
    declcode << "@use \"undef\";\n";
  }
...
bool PrepEngine::handle_import(const Token &input_tok) {
  ...
  LOG(DEBUG) << "Searching for file: {} in include directories [{}]" << filename << include_path
             << tok << std::endl;
  ...
  if (filepath.empty()) {
    LOG(ERROR) << "Could not find file: \"{}\" in include directories [{}]" << filename
  ...
  if (m_already_included.contains(filepath)) {
    LOG(WARN) << core::feedback[PREP_DUPLICATE_IMPORT] << m_stack.top().path << filepath << tok
              << std::endl;
    return true;
  }
...
Token PrepEngine::read_token() {
  ...
  if (m_we_are_root && !job->version) {
    if (!tok.is(tMacr) || !tok.as<std::string>().starts_with("use ")) {
