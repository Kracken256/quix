...
struct GeneralDistributionStats {
  ...
  void compute(const std::vector<double> &data) {
    ...
    mean = median = mode = stddev = min = max = q1 = q3 = iqr = range =
        skewness = kurtosis = variance = sum = 0.0;
    ...
    if (data.size() % 2 == 0)
      median =
          (sorted_data[data.size() / 2 - 1] + sorted_data[data.size() / 2]) / 2;
    else
    ...
    double sum_dev = 0.0;
    for (auto &d : data) sum_dev += (d - mean) * (d - mean);

    variance = sum_dev / data.size();
    stddev = std::sqrt(variance);

    double sum_cubed_dev = 0.0;
    for (auto &d : data) sum_cubed_dev += (d - mean) * (d - mean) * (d - mean);
  ...
  void print() {
    std::cout << "\x1b[34;49;1m== " << dist_name
              << " Distribution Stats ==\x1b[0m" << std::endl;
    std::cout << "\x1b[33;49mMean\x1b[0m:  \t\t\x1b[36;49m" << mean
              << "\x1b[0m\n";
    std::cout << "\x1b[33;49mMedian\x1b[0m: \t\x1b[36;49m" << median
              << "\x1b[0m\n";
    std::cout << "\x1b[33;49mMode\x1b[0m: \t\t\x1b[36;49m" << mode
              << "\x1b[0m\n";
    std::cout << "\x1b[33;49mStdDev\x1b[0m: \t\x1b[36;49m" << stddev
              << "\x1b[0m\n";
    ...
    std::cout << "\x1b[33;49mRange\x1b[0m: \t\t\x1b[36;49m" << range
              << "\x1b[0m\n";
    std::cout << "\x1b[33;49mSkewness\x1b[0m: \t\x1b[36;49m" << skewness
              << "\x1b[0m\n";
    std::cout << "\x1b[33;49mKurtosis\x1b[0m: \t\x1b[36;49m" << kurtosis
              << "\x1b[0m\n";
    std::cout << "\x1b[33;49mVariance\x1b[0m: \t\x1b[36;49m" << variance
              << "\x1b[0m\n";
...
struct BenchStats {
  ...
  GeneralDistributionStats ct_throughput;
};
...
static bool compile_single_source(std::vector<uint8_t> &out) {
  ...
  auto &compiler = quixcc::CompilerBuilder()
                       .add_code(libquixcc::benchmark_source,
                                 libquixcc::benchmark_source_len)
                       .set_output(outf)
                       .opt("-S")
...
static BenchStats run_bench() {
  ...
  for (size_t i = 0; i < num_iterations; i++) {
    ...
    if (i % (num_iterations / 20) == 0) {
      ...
      std::cout
          << "\x1b[36;49;1m*\x1b[0m \x1b[36;4mProgress: \x1b[0m\x1b[36;1;4m"
          << percent << "%\x1b[0m\tIteration " << i << "\t took " << duration
  ...
  for (auto &d : copy_ct_data) d /= libquixcc::benchmark_source_len * 8;
  ...
  for (auto &d : copy_ct_data) d /= out.size() * 8;
  bench.ct_per_target_byte.compute(copy_ct_data);

  /* Throughput Mbps second */
  bench.ct_throughput.dist_name = "Compilation Throughput (Mbps)";
  copy_ct_data = ct_data;
  for (auto &d : copy_ct_data)
    d = (libquixcc::benchmark_source_len * 8) / (d / 1e9) / 1e6;
  bench.ct_throughput.compute(copy_ct_data);
...
int main() {
  ...
  std::cout << "\x1b[36;49;1m*\x1b[0m Running benchmarks for "
               "\x1b[36;1;4mlibquixcc\x1b[0m..."
            << std::endl;
  std::cout << "\x1b[36;49;1m*\x1b[0m Doing " << num_iterations << " iterations"
            << std::endl;
  std::cout << "\x1b[36;49;1m*\x1b[0m This may take a while. Status updates "
               "will be printed every 5% of the way\n"
            << std::endl;
  auto bench = run_bench();

  std::cout << "\x1b[36;49;1m*\x1b[0m \x1b[32;49;1mBenchmarks complete\x1b[0m"
            << std::endl;

  std::cout << "\x1b[36;49;1m*\x1b[0m Printing benchmark results..."
            << std::endl;
  std::cout << "\x1b[36;49;1m*\x1b[0m \x1b[32;49;1mTimes are measured in "
               "nanoseconds\x1b[0m\n"
            << std::endl;
  ...
  bench.ct_throughput.print();

  std::cout
      << "\x1b[36;49;1m*\x1b[0m \x1b[32;49;1mBenchmark results printed\x1b[0m"
      << std::endl;

  std::cout << "\x1b[36;49;1m*\x1b[0m Writing datasets to "
               "\x1b[36;1;4mct_data.json, ct_per_source_byte_data.json, "
               "ct_per_target_byte_data.json, ct_throughput_data.json\x1b[0m"
            << std::endl;
  ...
  bench.ct_throughput.write_dataset_json("ct_throughput_data.json");
  return 0;
}
