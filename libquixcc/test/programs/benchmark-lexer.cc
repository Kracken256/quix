...
static bool do_bench(size_t *total_toks) {
  ...
  if ((outf = open_memstream(&outbuf, &outlen)) == nullptr) return false;

  if ((code = fmemopen((void *)libquixcc::benchmark_lexer_source,
                       libquixcc::benchmark_lexer_source_len, "r")) == nullptr)
    throw std::runtime_error(
        "Failed to open code source"); /* Mem leak is ignored */
