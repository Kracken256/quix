...
@description "Libc bindings for QUIX.";
@language "en";
...
struct crc32 {
    ...
    fn init() => this.state = 0xFFFFFFFF;

    fn update(data: *u8, len: usize) {
        const tab = [
        ...
        for i = 0; i < len; i += 1 {
            x = tab[(x ^ data[i]) & 0xFF] ^ (x >> 8);
        }
        
        this.state = x;
    }

    fn final(): u32 => ret this.state ^= 0xFFFFFFFF;
...
pub "C" 
{ 
    fn crc32_init(): crc32 => ret crc32::new();

    fn crc32_update(ctx: *crc32, buf: !*u8, n: usize) {
        let crc: crc32 = *ctx;
        crc.update(buf, n);
        ctx.state = crc.state;
    }

    fn crc32_final(ctx: *crc32): u32 {
        let crc: crc32 = *ctx;
        ret crc.final();
    }

    fn crc32_sum(data: string): u32 {
        let crc: crc32;
        crc.init();
        crc.update(data, strlen(data));
        ret crc.final();
