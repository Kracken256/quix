...
#define RESULT_FILE "cxx_benchmark.csv"
...
static bool do_bench(std::chrono::system_clock::time_point &start,
  ...
  const char *options[] = {"-emit-cxx", NULL};
...
static void write_cxx_result_csv(const std::vector<double> &throughput) {
...
int qpkg::bench::run_benchmark_cxx_codegen() {
  Progress progress("CXX Codegen");
  ...
  for (size_t i = 0; i < ROUNDS; i++) {
    ...
    if (!do_bench(start, end)) {
      std::cerr << "Failed to run benchmark for cxx." << std::endl;
      return -1;
    }
  ...
  write_cxx_result_csv(throughput);
