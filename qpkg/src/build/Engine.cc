...
qpkg::build::Engine::Engine(const std::string &package_src,
                            const std::string &output, int jobs,
                            std::unique_ptr<cache::ICache> cache,
                            qpkg::build::BuildSecurityConfig security,
                            qpkg::build::Optimization optimization, bool debug,
                            bool verbose) {
...
std::optional<qpkg::conf::Config> qpkg::build::Engine::load_config(
  ...
  if (std::filesystem::exists(base / "qpkg.yaml"))
    ...
    LOG(core::ERROR)
        << "No configuration file found in package source directory"
        << std::endl;
    return std::nullopt;
  }
}

std::vector<std::string> qpkg::build::Engine::get_source_files(
    const std::filesystem::path &base) {
    ...
    if (std::filesystem::is_directory(abs)) {
      for (const auto &entry :
        ...
        if (entry.path().string().ends_with(".q") && entry.is_regular_file())
          source_code_files.push_back(
              std::filesystem::canonical(entry.path()).string());
    } else if (abs.string().ends_with(".q") &&
               std::filesystem::is_regular_file(abs)) {
...
static qpkg::cache::CacheKey compute_cachekey(
  ...
  hasher.update(reinterpret_cast<const uint8_t *>(file.string().c_str()),
                file.string().size());

  return hasher.finalize();
}

bool qpkg::build::Engine::build_source_file(
    const std::filesystem::path &base, const std::filesystem::path &build_dir,
    const std::filesystem::path &file) const {
  cache::CacheKey key;
  std::string outfile =
      build_dir.string() + "/" + file.filename().string() + ".o";
  ...
  builder.set_verbosity(m_verbose ? quixcc::Verbosity::VERY_VERBOSE
                                  : quixcc::Verbosity::NORMAL);
...
bool qpkg::build::Engine::link_objects(
...
bool qpkg::build::Engine::run_threads(
    const std::filesystem::path &base,
  ...
  for (i = 0; i < tcount; i++) {
    threads.push_back(std::thread([this, &base, &log, w, &build_dir,
                                   &source_files, i, tcount, &tainted]() {
...
bool qpkg::build::Engine::build_package(
    const std::filesystem::path &base,
    const std::vector<std::string> &source_files,
  ...
  if (!source_files.empty()) {
    if (!run_threads(base, source_files, build_dir, log)) {
      LOG(core::ERROR) << "Failed to build package " << m_package_name
                       << std::endl;
      return false;
    }
    ...
    for (const auto &file : source_files)
      objects.push_back(
          build_dir / (std::filesystem::path(file).filename().string() + ".o"));
