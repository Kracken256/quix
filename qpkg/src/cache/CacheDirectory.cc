...
bool qpkg::cache::DirectoryCache::init() {
  LOG(core::DEBUG) << "Initializing cache directory: " << m_cacheDir
                   << std::endl;

  try {
    if (!std::filesystem::create_directories(m_cacheDir)) {
      LOG(core::ERROR) << "Failed to create cache directory: " << m_cacheDir
                       << std::endl;
      return false;
    }
    ...
    if (!record.is_open()) {
      LOG(core::ERROR) << "Failed to create cache index file: " << record_file
                       << std::endl;
      return false;
    }
    ...
    if (!std::filesystem::create_directories(m_cacheDir / "data")) {
      LOG(core::ERROR) << "Failed to create cache data directory: "
                       << m_cacheDir / "data" << std::endl;
      return false;
    }
...
bool qpkg::cache::DirectoryCache::discover() {
  LOG(core::DEBUG) << "Discovering cache directory: " << m_cacheDir
                   << std::endl;
  ...
  try {
    ...
    if (record.readsome(header, sizeof(header)) != sizeof(header) ||
      ...
      LOG(core::ERROR) << "Invalid cache index file: " << record_file
                       << std::endl;
      return false;
    }

    Record entry;
    while (record.readsome(reinterpret_cast<char *>(&entry), sizeof(entry)) ==
           sizeof(entry)) {
      CacheKey key;
      ...
      m_cacheMap[key] = std::make_pair(
          filepath, std::chrono::system_clock::from_time_t(entry.timestamp));
      m_keys.insert(key);
    }

    LOG(core::DEBUG) << "Found " << m_cacheMap.size() << " cache entries"
                     << std::endl;

    return true;
  } catch (const std::exception &e) {
    LOG(core::ERROR) << "Failed to discover cache directory: " << m_cacheDir
                     << std::endl;
    return false;
  }
}

bool qpkg::cache::DirectoryCache::setup(const std::string &location,
                                        bool init) {
...
void qpkg::cache::DirectoryCache::loadb(const CacheKey &key,
  ...
  try {
    ...
    if (!file.is_open()) {
      LOG(core::FATAL) << "Failed to open cache file: " << m_cacheMap[key].first
                       << std::endl;
    }
...
void qpkg::cache::DirectoryCache::storeb(const CacheKey &key,
...
void qpkg::cache::DirectoryCache::storef(const CacheKey &key,
...
void qpkg::cache::DirectoryCache::sync() {
  ...
  try {
    ...
    LOG(core::DEBUG) << "Synced " << m_cacheMap.size() << " cache entries"
                     << std::endl;
