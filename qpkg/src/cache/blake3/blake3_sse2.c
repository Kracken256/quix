...
INLINE __m128i rot16(__m128i x) { return _mm_shufflehi_epi16(_mm_shufflelo_epi16(x, 0xB1), 0xB1); }

INLINE __m128i rot12(__m128i x) { return xorv(_mm_srli_epi32(x, 12), _mm_slli_epi32(x, 32 - 12)); }

INLINE __m128i rot8(__m128i x) { return xorv(_mm_srli_epi32(x, 8), _mm_slli_epi32(x, 32 - 8)); }
...
void blake3_compress_in_place_sse2(uint32_t cv[8], const uint8_t block[BLAKE3_BLOCK_LEN],
...
void blake3_compress_xof_sse2(const uint32_t cv[8], const uint8_t block[BLAKE3_BLOCK_LEN],
...
static void blake3_hash4_sse2(const uint8_t *const *inputs, size_t blocks, const uint32_t key[8],
...
INLINE void hash_one_sse2(const uint8_t *input, size_t blocks, const uint32_t key[8],
...
void blake3_hash_many_sse2(const uint8_t *const *inputs, size_t num_inputs, size_t blocks,
  ...
  while (num_inputs >= DEGREE) {
    blake3_hash4_sse2(inputs, blocks, key, counter, increment_counter, flags, flags_start,
                      flags_end, out);
