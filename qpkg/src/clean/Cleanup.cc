...
static std::optional<qpkg::conf::Config> get_config(
  ...
  if (std::filesystem::exists(base / "qpkg.yaml")) {
    ...
    if (!c) {
      LOG(qpkg::core::ERROR)
          << "Failed to parse configuration file: " << base / "qpkg.yaml"
          << std::endl;
      return std::nullopt;
    }
...
bool qpkg::clean::CleanPackageSource(const std::string &package_src,
                                     bool verbose) {
  std::filesystem::path package_src_path(package_src);

  if (!std::filesystem::exists(package_src_path)) {
    LOG(core::ERROR) << "Package source path does not exist: " << package_src
                     << std::endl;
    return false;
  }

  if (!std::filesystem::is_directory(package_src_path)) {
    LOG(core::ERROR) << "Package source path is not a directory: "
                     << package_src << std::endl;
    return false;
  }
...
#define RMFILE(_file)                                                \
  ...
  if (std::filesystem::is_regular_file(package_src_path / tmp)) {    \
    if (verbose)                                                     \
      LOG(core::INFO) << "Removing file: " << package_src_path / tmp \
                      << std::endl;                                  \
