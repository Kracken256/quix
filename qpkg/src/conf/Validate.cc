...
bool qpkg::conf::ValidateConfig(const qpkg::conf::Config &config,
    ...
    switch (key_map[key]) {
      case KeyName::NAME:
        if (!config[key].is<std::string>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'name' in configuration"
              << std::endl;
          return false;
        }
      ...
      case KeyName::VERSION:
        if (!config[key].is<int64_t>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'version' in configuration"
              << std::endl;
          return false;
        }
        if (config[key].as<int64_t>() != 1) {
          LOG(core::ERROR)
              << "This version of the QPKG system only supports version 1"
              << std::endl;
          return false;
        }
        break;
      ...
      case KeyName::AUTHORS:
        if (!config[key].is<std::vector<std::string>>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'authors' in configuration"
              << std::endl;
          return false;
        }
      ...
      case KeyName::EMAILS:
        if (!config[key].is<std::vector<std::string>>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'emails' in configuration"
              << std::endl;
          return false;
        }
      ...
      case KeyName::URL:
        if (!config[key].is<std::string>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'url' in configuration"
              << std::endl;
          return false;
        }
        if (!is_utf8(config[key].as<std::string>().c_str())) {
      ...
      case KeyName::LICENSES:
        ...
        for (const auto &license : config[key].as<std::vector<std::string>>()) {
          if (!qpkg::conf::valid_licenses.contains(license)) {
            LOG(core::ERROR)
                << "Invalid license in configuration: " << license << std::endl;
            return false;
          }
        }
        break;
      case KeyName::SOURCES:
        if (!config[key].is<std::vector<std::string>>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'sources' in configuration"
              << std::endl;
          return false;
        }
        for (const auto &source : config[key].as<std::vector<std::string>>()) {
          if (!std::filesystem::exists(base / source)) {
            LOG(core::ERROR)
                << "Source does not exist: " << source << std::endl;
            return false;
          }
        }
        break;
      case KeyName::HEADERS:
        if (!config[key].is<std::vector<std::string>>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'headers' in configuration"
              << std::endl;
          return false;
        }
        for (const auto &source : config[key].as<std::vector<std::string>>()) {
          if (!std::filesystem::exists(base / source) ||
            ...
            LOG(core::ERROR)
                << "Directory does not exist: " << source << std::endl;
            return false;
          }
        }
        break;
      case KeyName::TARGET:
        if (!config[key].is<std::string>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'target' in configuration"
              << std::endl;
          return false;
        }
      ...
      case KeyName::TRIPLE:
        if (!config[key].is<std::string>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'triple' in configuration"
              << std::endl;
          return false;
        }
        if (!is_utf8(config[key].as<std::string>().c_str())) {
      ...
      case KeyName::CPU:
        if (!config[key].is<std::string>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'cpu' in configuration"
              << std::endl;
          return false;
        }
        if (!is_utf8(config[key].as<std::string>().c_str())) {
      ...
      case KeyName::CFLAGS:
        if (!config[key].is<std::vector<std::string>>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'cflags' in configuration"
              << std::endl;
          return false;
        }
        break;
      case KeyName::LFLAGS:
        if (!config[key].is<std::vector<std::string>>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'lflags' in configuration"
              << std::endl;
          return false;
        }
        break;
      case KeyName::NOLINK:
        if (!config[key].is<bool>()) {
          LOG(core::ERROR)
              << "Invalid value type for key 'nolink' in configuration"
              << std::endl;
          return false;
        }
        break;
      case KeyName::PACKAGES:
        ...
        for (const auto &source : config[key].as<std::vector<std::string>>()) {
          if (!std::filesystem::exists(base / source) ||
              !std::filesystem::is_directory(base / source)) {
            LOG(core::ERROR)
                << "Dependency does not exist: " << source << std::endl;
            return false;
          }
        }
        break;
      default:
        LOG(core::ERROR) << "Invalid key in configuration: " << key
                         << std::endl;
        return false;
    }
  }
  ...
  for (const auto &key : required_keys) {
    if (!keys.contains(key)) {
      LOG(core::ERROR) << "Missing required key in configuration: " << key
                       << std::endl;
      return false;
    }
  }
