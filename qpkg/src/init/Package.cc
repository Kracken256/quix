...
bool qpkg::init::Package::createPackage()
{
    ...
    try
    {
        ...
        if (system(("git init " + (m_output / m_name).string()).c_str()) != 0)
        {
...
{
    return qpkg::init::Package(m_output, m_name, m_license, m_author, m_email, m_url, m_version, m_type, m_verbose, m_force);
}